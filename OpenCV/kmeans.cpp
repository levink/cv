    ...
    for (int ii = 0; ii < k; ii++) 
      {
        ...
        if(DEBUG) printf("WARNING: Empty cluster %d! \n", ii);
...
void  perform_move(int dim, int n, int k, float *X, int *cluster_assignment, float *cluster_centroid, int *cluster_member_count, int move_point, int move_target_cluster)
  {
   ...
   // update cluster count array
    ...
    if (cluster_member_count[cluster_old] <= 1)
     if(DEBUG) printf("WARNING: Can't handle single-member clusters! \n");
...
void cluster_diag(int dim, int n, int k, float *X, int *cluster_assignment_index, float *cluster_centroid)
  {
   ...
   if(DEBUG) printf("  Final clusters \n");
    for (int ii = 0; ii < k; ii++) 
    if(DEBUG) printf("    cluster %d:     members: %8d, centroid (%.1f %.1f) \n", ii, cluster_member_count[ii], cluster_centroid[ii*dim + 0], cluster_centroid[ii*dim + 1]);
  }
...
void kmeans(
  ...
  {
    float *dist                    = new float[n*k];  //(float *)malloc(sizeof(float) * n * k);
    int   *cluster_assignment_cur  =  new int[n]; //(int *)malloc(sizeof(int) * n);
    int   *cluster_assignment_prev =  new int[n]; //(int *)malloc(sizeof(int) * n);
    float *point_move_score        = new float[n*k];//(float *)malloc(sizeof(float) * n * k);
...
//        cluster_diag(dim, n, k, X, cluster_assignment_cur, cluster_centroid);
        ...
        // move all points to nearest cluster
         ...
         if(DEBUG) printf("%3d   %u   %9d  %16.2f %17.2f\n", batch_iteration, 1, change_count, totD, totD - prev_totD);
         fflush(stdout);
         
        // done with this phase if nothing has changed
         if (change_count == 0)
           {
             if(DEBUG) printf("  no change made on this step - iteration completed \n");
             break;
           }
...
/* The online update prtion of this code has never worked properly, but batch update has been adequate for our projects so far.
    ...
    while (online_iteration < MAX_ITERATIONS)
      {
//        if(DEBUG) printf("online iteration %d \n", online_iteration);
        ...
        for (int ii = 0; ii < n; ii++)
          {
           ...
           // find the best target for it
            ...
            float best_delta        = BIG_float;
            
           // for each possible target
            for (int jj = 0; jj < k; jj++)
              {
                float cur_delta = point_move_score[point_to_consider*k + jj];
...
//    if(DEBUG) printf("iterations: %3d %3d \n", batch_iteration, online_iteration);
